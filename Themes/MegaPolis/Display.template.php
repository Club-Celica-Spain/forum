...
<tr>
<td class="middletext" valign="bottom" style="padding-bottom: 4px;">', $txt[139], ': ', $context['page_index'], !empty($modSettings['topbottomEnable']) ? $context['menu_separator'] . ' <a href="https://'.$_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"].'#lastPost"><b>' . $txt['topbottom5'] . '</b></a>' : '', '</td> <td align="right" style="padding-right: 1ex;">
	...
	<tr><td style="padding: 1px 1px 0 1px;">';
		...
		echo '
				<table width="100%" cellpadding="5" cellspacing="0" style="table-layout: fixed;">
					<tr>
						<td valign="top" width="16%" rowspan="2" style="overflow: hidden;"><div class="prof2"><div class="prof1">
							<b>', $message['member']['link'], '</b></div>
							<div class="smalltext">';
		...
		if (!$message['member']['is_guest'])
		{
			...
			if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
				echo '
								<br />', $message['member']['avatar']['image'], '<br />';
			...
			if ($modSettings['karmaMode'] == '1')
				echo '
								...
								<div class="prof">', $modSettings['karmaLabel'], ' ', $message['member']['karma']['good'] - $message['member']['karma']['bad'], '';
			...
			if ($message['member']['karma']['allow'])
				echo '
								<br /><a href="', $scripturl, '?action=modifykarma;sa=applaud;uid=', $message['member']['id'], ';topic=', $context['current_topic'], '.' . $context['start'], ';m=', $message['id'], ';sesc=', $context['session_id'], '">', $modSettings['karmaApplaudLabel'], '</a>
			...
			if (!empty($settings['show_gender']) && $message['member']['gender']['image'] != '')
				echo '<div class="prof">
								', $txt[231], ': ', $message['member']['gender']['image'], '</div>';

						
			// Show how many posts they have made.
			echo '<div class="prof">
								', $txt[26], ': ', $message['member']['posts'], '</div>
								';
			//Mostrar Nº Socio
			$idUser=$message['member']['id'];
			$consulta = db_query("SELECT Socio,LPAD(ID_MEMBER,4,0)numSocio FROM smf_members WHERE ID_MEMBER=$idUser");
			$dato=mysql_fetch_assoc($consulta);
			$socio=$dato["Socio"];
			if ($socio == '1'){
				//echo '<div class="prof">', $socio,'','';
				echo '<div class="prof">
					', 'N&deg; de Socio', ': ', $dato['numSocio'], '</div>';
			}	
			//Mostrar fecha registro
			$mostrarfecha1 = $message ['member']['registered'];
			$mostrarfecha2 = substr ($mostrarfecha1, 0, -10);
			
			//INICIO: Codigo nuevo 27/09/2010
			
			//Por ejemplo Marzo 30, 2009 lo pasamos a 30 de marzo del 2009
			
			$partePrimera=explode(",",$mostrarfecha2);
			//Aqui tenemos vector, en $camposPartePrimera[0]=Marzo, en $camposPartePrimera[1]=30
			$camposPartePrimera=explode(" ",$partePrimera[0]);
			//Mes
			$mes=$camposPartePrimera[0];
			//Dia
			$dia=$camposPartePrimera[1];
			
			//Quitamos los espacios del inicio de la cadena
			$parteSegunda=trim($partePrimera[1]);
			$anyo=$parteSegunda;
			
			$fechaFormateadaFinal=$dia." de ".$mes." del ".$anyo;
			
			
			//FIN: codigo nuevo 27/09/2010
			
			echo '<div class="prof">
								', $fechaFormateadaFinal, '</div>';
		...
		// Show Facebook icon
			if (!empty($settings['display_facebook_like'])) {
				if ($message['id'] == $context['topic_first_message'] || !empty($settings['display_facebook_like_all'])) {
				echo '<iframe src="https://www.facebook.com/plugins/like.php?href=', $scripturl, '?topic=', $context['current_topic'], '&amp;layout=button_count&amp;show_faces=true&amp;width=80&amp;action=like&amp;colorscheme=light" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:100px; height:21px"></iframe>';
				}
			}
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
		<td class="middletext">', $txt[139], ': ', $context['page_index'], !empty($modSettings['topbottomEnable']) ? $context['menu_separator'] . ' &nbsp;&nbsp;<a href="https://'.$_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"].'#top"><b>' . $txt['topbottom4'] . '</b></a>' : '', '</td>
