...
function print_screen($current_plugin_version, $mobiquo_config)
{
    require_once(dirname(dirname(__FILE__)) . '/SSI.php');
    global $boardurl;
	$latest_tp_plugin_version = 'sm-2a_'.get_latest_plugin_version();
	$check_upload_status = file_get_contents($boardurl."/mobiquo/upload.php?checkAccess");
	$check_push_status = file_get_contents($boardurl."/mobiquo/push.php?checkAccess");
	global $modSettings;
	...
	echo "Latest Tapatalk plugin version: <a href=\"https://www.tapatalk.com/download_SimpleMachines.php\" target=\"_blank\">".$latest_tp_plugin_version."</a><br>";

	echo "Attachment upload interface status: <a href=\"".$boardurl."/mobiquo/upload.php\" target=\"_blank\">".($check_upload_status ? 'OK' : 'Inaccessible')."</a><br>";

	echo "Push notification interface status: <a href=\"".$boardurl."/mobiquo/push.php\" target=\"_blank\">".($check_push_status ? 'OK' : 'Inaccessible')."</a><br>";
