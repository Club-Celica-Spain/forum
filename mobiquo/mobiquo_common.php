...
function get_error($err_str = '')
{
    ...
    if (isset($tt_sso_result) &&!empty($tt_sso_result)){
        $result['register'] = new xmlrpcval($tt_sso_result->register, 'boolean');
...
function post_html_clean($str)
{
    ...
    if(isset($modSettings['tp_custom_content_replacement']) && !empty($modSettings['tp_custom_content_replacement']))
    {
        ...
        $custom_replacement = preg_replace('/^\s*#.*?$/m', '', $custom_replacement);
        $custom_replacement = trim($custom_replacement);
...
function get_user_by_id($uid)
{
    ...
    $request = $smcFunc['db_query']('', '
        SELECT passwd, member_name, id_member, id_group, lngfile, is_activated, email_address, additional_groups, member_name, password_salt, openid_uri, email_address AS email,
        passwd_flood, validation_code
        FROM {db_prefix}members
        WHERE id_member = {int:current_member}
        LIMIT 1',
        array(
            'current_member' => intval($uid),
        )
    );
...
function get_user_by_name_or_email($name, $is_email = false)
{
    ...
    {
        $request = $smcFunc['db_query']('', '
           SELECT passwd, member_name, id_member, id_group, lngfile, is_activated, email_address, additional_groups, member_name, password_salt, email_address AS email,
