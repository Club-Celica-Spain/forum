...
function get_config_func()
{
    ...
    if(allowedTo('who_view') && $modSettings['who_enabled'] )
    ...
    if ($user_info['is_guest'] && allowedTo('who_view') && $modSettings['who_enabled'] )
        $config_list['guest_whosonline'] = new xmlrpcval('1', 'string');
...
function login_func()
{
    global $context, $user_info, $user_profile, $settings, $scripturl, $modSettings, $tt_sso_result;
    ...
    $login_status = ($context['user']['is_guest'] || (isset($context['disable_login_hashing']) && $context['disable_login_hashing'])) ? false : true;
    $result_text = (!$login_status && isset($context['login_errors'][0])) ? basic_clean($context['login_errors'][0]).' '.(isset($context['login_errors'][1])? basic_clean($context['login_errors'][1]) : '') : '';
    ...
    $user_type = isset($context['user_type']) ? $context['user_type'] : tt_get_user_type($user_info['id']);
    ...
    $response = array(
        ...
        'email'         => new xmlrpcval(sha1(strtolower($profile['email_address'])), 'base64'),
        ...
        'can_whosonline'    => new xmlrpcval(allowedTo('who_view') && $modSettings['who_enabled'], 'boolean'),
        'post_countdown'    => new xmlrpcval(allowedTo('moderate_board') ? 0 : $modSettings['spamWaitTime'], 'int'),
    );

    if($tt_sso_result->register)
    {
        $response['register'] =  new xmlrpcval(true, 'boolean');
    }
...
function get_user_info_func()
{
    ...
    if (!empty($context['member']['group']) || !empty($context['member']['post_group'])) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['position']), 'base64'),
    ...
    if ($context['member']['show_email'] == 'yes' || $context['member']['show_email'] == 'yes_permission_override') {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['email']), 'base64'),
    ...
    if ($context['member']['website']['url'] != '' && !isset($context['disabled_fields']['website'])) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['website_title']), 'base64'),
    ...
    if (!isset($context['disabled_fields']['icq']) && !empty($context['member']['icq']['link'])) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['icq']), 'base64'),
    ...
    if (!isset($context['disabled_fields']['msn']) && !empty($context['member']['msn']['link'])) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['msn']), 'base64'),
    ...
    if (!isset($context['disabled_fields']['aim']) && !empty($context['member']['aim']['link'])) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['aim']), 'base64'),
    ...
    if (!isset($context['disabled_fields']['yim']) && !empty($context['member']['yim']['link'])) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['yim']), 'base64'),
    ...
    if (!empty($modSettings['titlesEnable']) && !empty($context['member']['title'])) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['custom_title']), 'base64'),
    ...
    if (!isset($context['disabled_fields']['gender']) && !empty($context['member']['gender']['name'])) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['gender']), 'base64'),
            'value' => new xmlrpcval(to_utf8($context['member']['gender']['name']), 'base64')
        ), 'struct');
    }

    if ($context['member']['age']) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['age']), 'base64'),
    ...
    if (!isset($context['disabled_fields']['location']) && !empty($context['member']['location'])) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['location']), 'base64'),
    ...
    if ($context['can_view_warning'] && $context['member']['warning']) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['profile_warning_level']), 'base64'),
    ...
    if ($context['can_see_ip']) {
        if (!empty($context['member']['ip'])) {
            $custom_fields[] = new xmlrpcval(array(
                'name'  => new xmlrpcval(to_utf8($txt['ip']), 'base64'),
        ...
        if (empty($modSettings['disableHostnameLookup']) && !empty($context['member']['ip'])) {
            $custom_fields[] = new xmlrpcval(array(
                'name'  => new xmlrpcval(to_utf8($txt['hostname']), 'base64'),
    ...
    if (!empty($modSettings['userLanguage']) && !empty($context['member']['language'])) {
        $custom_fields[] = new xmlrpcval(array(
            'name'  => new xmlrpcval(to_utf8($txt['language']), 'base64'),
...
{
    global $context, $txt, $tt_sso_result;
...
function prefetch_account_func()
{
    ...
    if (isset($context['profile_fields']) && !empty($context['profile_fields'])){
        foreach ($context['profile_fields'] as $key => $profile_field){
            ...
            if (empty($field_option) && $profile_field['type'] == 'check'){
            ...
            $custom_register_fields[] = new xmlrpcval( array(
                'name'        => new xmlrpcval(basic_clean($profile_field['label']), 'base64'),
                'description' => new xmlrpcval(basic_clean($description), 'base64'),
                'key'         => new xmlrpcval($key, 'string'),
                'type'        => new xmlrpcval($field_type, 'string'),
                'options'     => new xmlrpcval(basic_clean($field_option), 'base64'),
                'default'     => new xmlrpcval(basic_clean($default), 'base64'),
    ...
    while ($custom_field = $smcFunc['db_fetch_assoc']($request)){
        ...
        $custom_register_fields[] = new xmlrpcval(array(
            'name'        => new xmlrpcval(basic_clean($custom_field['field_name']), 'base64'),
            'description' => new xmlrpcval(basic_clean($custom_field['field_desc']), 'base64'),
...
{
    global $context, $smcFunc, $user_info, $profile, $user_profile, $settings, $scripturl, $modSettings;
