...
function setLoginCookie($cookie_length, $id, $password = '')
{
	...
	if (!isset($_SESSION['login_' . $cookiename]) || $_SESSION['login_' . $cookiename] !== $data)
	{
		...
		if (version_compare(PHP_VERSION, '4.3.2') === 0)
		{
			...
			setcookie(session_name(), session_id(), time() + (empty($sessionCookieLifetime) ? $cookie_length : $sessionCookieLifetime), $cookie_url[1], $cookie_url[0], !empty($modSettings['secureCookies']), true);
		}
